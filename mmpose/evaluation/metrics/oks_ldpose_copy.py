    ...
    def computeOks(self, imgId, catId):
        ...
        for j, gt in enumerate(gts):
            ...
            for i, dt in enumerate(dts):
                ...
                def _exists(score: float, others):
                    return self._exists(score, others, self.exist_mode, self.exist_threshold)

                gamma = np.ones(k, dtype=float)
                for anchor, others in self.conflict_map.items():
                    a_exist = _exists(p['sc'][anchor], [p['sc'][i] for i in others])
                    others_exist = any(_exists(p['sc'][i], [p['sc'][anchor]]) for i in others)
