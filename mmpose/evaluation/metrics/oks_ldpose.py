from typing import List, Dict, Sequence, Optional, Literal
import numpy as np
from mmengine.logging import MMLogger

from mmpose.evaluation.metrics.coco_metric import CocoMetric
from mmpose.registry import METRICS
from ..functional import (oks_nms, soft_oks_nms, transform_pred)
import datetime
import time
...
class CocoMetricLD(CocoMetric):
    """在 COCO 指标之外，额外计算 OKS-LD（带 limb 互斥惩罚 γ）。

    额外参数：
        conflict_map (Dict[int, List[int]]): 互斥关系图。key 通常是残端点，
            value 是与之互斥的自然关节索引列表。
            例：{21:[13,15,23], 22:[14,16,24], 17:[7,9], 19:[7,9], ...}
        exist_mode (str): 'threshold' or 'relative'。默认 'threshold'。
        exist_threshold (float): 置信度阈值（exist_mode='threshold' 时使用）。
        oks_thresholds (Sequence[float] | None): 若给出，额外算 mOKS@t。
        ignore_gt_invisible (bool): 若 True，则对 GT 不可见点 γ=0。
        ld_log_prefix (str): 结果前缀，默认 'ldpose'。
    """
    ...
    def __init__(self,
        ...
        self.conflict_map = {int(k): list(map(int, v)) for k, v in conflict_map.items()}
        ...
        for _, img_kpts in keypoints.items():
            ...
            for img_kpt, keypoint in zip(img_kpts, _keypoints):
                res = {
                    ...
                    'keypoint_scores': img_kpt['keypoint_scores'],
                }
                if 'bbox' in img_kpt:
...
class COCOLDeval:
    ...
    def _exists(self, score: float, thr: float) -> bool:
    ...
    def computeOks(self, imgId, catId):
        ...
        for j, gt in enumerate(gts):
            ...
            for i, dt in enumerate(dts):
                ...
                def _exists(score: float):
                    return self._exists(score, self.exist_threshold)

                gamma = np.ones(k, dtype=float)
                for anchor, others in self.conflict_map.items():
                    a_exist = _exists(dt['keypoint_scores'][anchor])
                    others_exist = any(_exists(dt['keypoint_scores'][i]) for i in others)
                    if a_exist and others_exist:
                        gamma[anchor] = 0.0

                exp_term = np.exp(-e)
                if k1 > 0:
                    vis = (vg > 0)
                    num = (gamma[vis] * exp_term[vis]).sum()
                    den = vis.sum()
                    ious[i, j] = num / den if den > 0 else 0.0
                else:
